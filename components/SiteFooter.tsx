  ...
  return (
    <footer className="bg-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 py-14">
            ...
            <div className="flex flex-col gap-2.5">
              {[
                ...
                <Link
                  key={link.label}
                  href={link.href}
                  className="text-sm text-slate-300 transition-colors hover:text-cyan-400 hover:translate-x-0.5 inline-block"
                >
                  {link.label}
                </Link>
              ))}
        ...
        <div className="mt-12 pt-6 border-t border-slate-600 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-slate-400">
            &copy; {new Date().getFullYear()} Devidends &mdash; Envest
            Technologies PLC. All rights reserved.
          </p>
          <div className="flex items-center gap-1">
            <span className="text-xs text-slate-400">Built for the</span>
