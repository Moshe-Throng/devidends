...
## Paid API Spending Rules — CRITICAL

Before calling ANY paid API (fal.ai, Replicate, ElevenLabs, Suno API, etc.):
1. **Estimate the cost** and tell the user before submitting
2. **Check if balance is sufficient** — if a previous call returned a balance/billing error, do NOT retry without asking
3. **Verify results** — check file sizes (>500 bytes), don't mark 0-byte files as "done"
4. **Never auto-retry more than once** — report failure and ask before spending more
5. **Track running total** — periodically tell the user how much has been spent this session
6. **Prefer free alternatives** — Gemini via OpenRouter for images, PIL for text, moviepy for assembly
7. **Never batch-test multiple paid models** without confirming total cost with the user first
