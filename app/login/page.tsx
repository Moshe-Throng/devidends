...
function LoginContent() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      const { error: oauthErr } = await supa.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(searchParams.get("next") || "/opportunities")}`,
        },
      });
      if (oauthErr) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "forgot") {
      ...
      if (err) setError(err);
      else router.push(searchParams.get("next") || "/opportunities");
    } else {
      const { error: err } = await signUpWithEmail(email, password);
      if (err) setError(err);
      else setSuccessMsg("Check your email for a confirmation link, then log in.");
    }
  ...
  const subtext =
    ...
    mode === "login"  ? "Sign in to save opportunities and track your applications" :
      ...
      <main className="flex-1 flex items-center justify-center px-5 py-16">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-extrabold text-dark-900 tracking-tight">{heading}</h1>
          ...
          <div className="bg-white rounded-2xl border border-dark-100 p-8 shadow-sm">
            ...
            {mode !== "forgot" && (
              <div className="mb-6 space-y-3">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl border border-dark-100 bg-white text-sm font-bold text-dark-900 transition hover:bg-dark-50 disabled:opacity-50"
                >
                  {googleLoading ? (
                    ...
                    <>
                      <svg className="w-4 h-4" viewBox="0 0 48 48" aria-hidden>
                        <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.7-6.1 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 7.9 3l5.7-5.7C34.1 6 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.3-.4-3.5z"/>
                        <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.7 16 19 13 24 13c3.1 0 5.8 1.1 7.9 3l5.7-5.7C34.1 6 29.3 4 24 4 16.3 4 9.7 8.4 6.3 14.7z"/>
                        <path fill="#4CAF50" d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3c-2 1.4-4.5 2.2-7.3 2.2-5.2 0-9.6-3.3-11.3-8l-6.5 5C9.6 39.5 16.2 44 24 44z"/>
                        <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.2 4.2-4 5.5l6.3 5.3C41.9 35.1 44 29.9 44 24c0-1.3-.1-2.3-.4-3.5z"/>
                      </svg>
                ...
                <div className="relative py-2">
                  ...
                  <div className="relative flex justify-center text-xs uppercase tracking-wider text-dark-300">
                    <span className="bg-white px-3 font-bold">or email</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {successMsg && (
                <p className="text-sm text-emerald-700 font-medium bg-emerald-50 rounded-lg px-3 py-2">{successMsg}</p>
              )}
              ...
              <p className="text-center text-sm text-dark-400 mt-6">
                {mode === "login" ? (
                  <>Don&apos;t have an account?{" "}
                    <button onClick={() => switchMode("signup")} className="text-cyan-600 font-semibold hover:text-cyan-700">
                      Sign up
                    </button>
                  </>
                ) : (
                  <>Already have an account?{" "}
                    <button onClick={() => switchMode("login")} className="text-cyan-600 font-semibold hover:text-cyan-700">
                      Sign in
                    </button>
                  </>
                )}
          ...
          <p className="text-center text-xs text-dark-300 mt-5">
            ...
            <Link href="#" className="underline hover:text-dark-500">Terms</Link> and{" "}
            <Link href="#" className="underline hover:text-dark-500">Privacy Policy</Link>
          </p>
        </div>
      </main>
